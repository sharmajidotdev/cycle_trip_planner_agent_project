...
class CyclingTripAgent:
    ...
    async def _call_llm(self, messages: List[Dict[str, Any]], tools: List[Dict[str, Any]]) -> Any:
        if not self.client:
    ...
    async def chat(self, conversation_id: str, user_message: str) -> dict:
        ...
        first_response = await self._call_llm(messages=messages, tools=self.tool_specs)
        ...
        if tool_results_payload:
            ...
            final_response = await self._call_llm(messages=messages, tools=self.tool_specs)
            content_blocks = [
